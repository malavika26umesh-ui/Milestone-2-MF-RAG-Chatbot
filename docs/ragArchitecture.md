...
Scheduler (GitHub Actions): A repository workflow runs on a cron schedule once per day at 09:15 IST (Asia/Kolkata) to execute the full ingest job so the Remote Chroma Cloud index reflects the latest scraped HTML from the allowlisted URLs. See §4.0 for workflow details, secrets, and manual workflow_dispatch.
Scraping service: On each triggered run, reads the URL registry, fetches every allowlisted page (HTTP(S), rate-limited, identifiable user-agent), persists raw HTML for audit/replay, and passes content into normalize → chunk → embed → remote index. It does not crawl beyond the registry.
Ingestion pipeline: Builds and refreshes the remote vector index and a parallel document registry (human-readable list of sources, fetch dates, scheme/AMC tags).
...
Near-term (file-based, next to raw HTML): After each ingest run, write e.g. data/structured/<run_id>/scheme_facts.json (or .jsonl one object per scheme). Commit is optional; production can upload the artifact next to data/raw/ or sync to object storage.
Production: Load the same schema into Postgres (row per scheme_id + fetched_at, or upsert “current” row per scheme) or a document store. Keep source_url and fetched_at on every record for citations and the “Last updated from sources” footer.
...
Workflow responsibilities: The scheduled job in .github/workflows/ingest.yml checks out the repo, installs dependencies, and runs in order: 4.0 scrape → 4.1 normalize → 4.2 chunk + embed (local BAAI/bge-small-en-v1.5) → 4.3 upsert to Remote Chroma Cloud (same run_id throughout). See chunking-embedding-architecture.md for chunk/embed details.
Secrets: Embedding uses local BAAI/bge-small-en-v1.5 (no API key). Chroma Cloud: vectors are upserted via `HttpClient` using `CHROMA_HOST`, `CHROMA_API_KEY`, `CHROMA_TENANT`, and `CHROMA_DATABASE` secrets. Local storage (`data/chroma/`) is only used as a development fallback.
...
Index (phase 4.3 — Remote Chroma Cloud)

Upsert vectors and metadata into Remote Chroma Cloud via HttpClient (see §4.3 below). Same embedding dimension as ingest (384 for bge-small-en-v1.5).
Refresh
...
### 4.3 Phase: Vector index — Remote Chroma Cloud (HttpClient)
Product choice: Use Chroma Cloud for dense retrieval with `chromadb.HttpClient`. This enables a managed, multi-tenant vector service accessible from both the ingestion pipeline and the runtime API.

Ingest-time steps (ordered)

#### Client & Authentication
HttpClient(host=..., port=443, ssl=True) configured via environment variables. Authentication is handled via Bearer Token using `CHROMA_API_KEY`.

#### Collection, Tenant, and Database
The service uses a specific Tenant and Database (e.g., `MFRAGdb`) to isolate data. The collection (e.g., `mutual_fund_faqs`) uses 384-dimensional vectors matching the BGE model.
Record shape (align with chunking-embedding-architecture.md §5)
...
For each daily ingest run: upsert by chunk_id (add new, update changed embeddings/metadata) into the remote collection.
...
If a scheme is removed from the URL registry, delete all collection entries whose scheme_id (or source_url) matches the removed scheme.
Alternatively replace collection on full reindex for small corpora.
Registry / operator manifest

Emit a small index manifest (JSON) per run: embedding_model_id, run_id, collection_name, remote_host, chunk_count, updated_at, indexed_at — so operators know which environment the app must target.
CI (GitHub Actions)

After phase 4.2: run phase 4.3 using Chroma Cloud secrets. The pipeline performs a remote upsert to the production instance.
Query-time (runtime API — future wiring)

Use the same HttpClient configuration as ingest.
Embed user query with the same BGE model and query prefix (Represent this sentence: ) per chunking doc.
collection.query(query_embeddings=[...], n_results=k, where={...}) with optional filters on scheme_id / amc.
Pass retrieved documents + metadata source_url into the LLM context packager (§6).

5. Retrieval Layer
Implementation (code): runtime/phase_5_retrieval/ — BGE query embedding, Chroma query against the remote store, merge by source_url, primary citation_url for §6. CLI: python -m runtime.phase_5_retrieval "…".
...
Dense retrieval: Top-k (e.g., 20–40) by cosine similarity in vector store.
Metadata filter: Optional pre-filter by scheme_id or amc when resolved.
Re-ranking (recommended): Cross-encoder or lightweight lexical re-rank on candidate chunks to improve table/number-heavy FAQ hits.
Merging: If multiple chunks from the same source_url score highly, merge text for context while keeping one citation URL.
...
Footer: Last updated from sources: <date> using corpus or cited document fetched_at (policy: max date across chunks used, or date of cited source only—pick one and document in README).
...
Vector DB	Chroma Cloud — phase 4.3 (HttpClient connection to remote instance; see §4.3)
...
14. Summary
The architecture is a closed-book RAG system: a curated, versioned corpus of allowlisted URLs (currently five Groww HDFC scheme pages, HTML only) is refreshed by a GitHub Actions schedule (09:15 IST) that runs scrape → normalize → chunk → embed (see chunking-embedding-architecture.md) → Remote Chroma Cloud vector upsert (§4.3); at query time, a router and retriever (connecting to the same remote collection) constrain what may be said, and prompts plus post-validation enforce how it is said—short, factual, one source link, and compliant refusal paths for non-factual or advisory requests.